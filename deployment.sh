...
if [[ "$1" == "" ]]; then
    ...
    title "2. Restore the dump in the local database"
        ...
        yes_no='Y'
        ask 'The local schema citizens-reserve:public will be dropped (Y/N)' yes_no
        ...
        (
            echo "DROP SCHEMA public CASCADE; CREATE SCHEMA public;" | psql -h localhost -U play -d citizens-reserve -w -q
        ) && success "done" || handle_error

        info "Restoring dump ... "
        (
            psql -h localhost -U play -d citizens-reserve -w -q < $dump_file > /dev/null
        ) && success "done" || handle_error
...
if [[ "$1" == "--restore-last" || "$1" == "--rl" ]]; then
    ...
    info -n "Dropping schema ... "
    (
        echo "DROP SCHEMA public CASCADE; CREATE SCHEMA public;" | psql -h localhost -U play -d citizens-reserve -p 5432 -w -q
    ) && success "done" || handle_error

    info "Restoring dump ... "
    (
        psql -h localhost -U play -d citizens-reserve -p 5432 -w -q < $dump_file > /dev/null
    ) && success "done" || handle_error
