...
angular.module('app.controllers').config(function($routeProvider) {
  ...
  return;
});angular.module('app.services').config(function($modalProvider) {
  return angular.extend($modalProvider.defaults, {
    html: true
  });
}).service("modalService", function($rootScope, $modal) {
  this.open = function(modalName, scope) {
    var myModal;
    return myModal = $modal({
      template: '$/angular/views/' + modalName + '.html',
      scope: scope
    });
  };
  return;
  ...
  return {
    ...
    link: function(scope) {
      ...
      return scope.computeOptionLabel = function(o) {
        if (!o) {
          return void 0;
        }
        if (o.icon) {
...
});angular.module('app.controllers').controller("MainCtrl", function($scope, translationService, modalService) {
  $scope.initialLoad = {
    translations: false
  };
  ...
  return $scope.m = function() {
    return modalService.open('nice-modal', $scope.$new({
      a: 12
    }));
  };
});
angular.module('app').run(function($rootScope, $location, translationService) {});angular.module('app.controllers').controller("FormCtrl", function($scope, $http, modalService, downloadService) {
  $scope.text = "example";
  $scope.open = function() {
    return modalService.show(modalService.TEST);
  };
